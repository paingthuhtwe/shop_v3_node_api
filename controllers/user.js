...
const register = async (req, res, next) => {
  try {
    ...
    if (dbUserPhone) {
      Helper.sendError(
        409,
        `Phone is already in use. Please choose a different phone number.`,
        next
      );
    }
...
const addRole = async (req, res, next) => {
  try {
    ...
    if (!dbUser) {
      Helper.sendError(
        400,
        "The requested user was not found in the user table.",
        next
      );
      return;
    }
    if (!dbRole) {
      Helper.sendError(
        400,
        "The requested role was not found in the role table.",
        next
      );
      return;
    }
    ...
    Helper.sendError(
      500,
      `An error occurred while adding the role: ${error.message}`,
      next
    );
  }
};
...
const removeRole = async (req, res, next) => {
  try {
    ...
    Helper.sendError(
      500,
      `An error occurred while removing the role: ${error.message}`,
      next
    );
  }
};
